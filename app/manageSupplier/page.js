...
export default function SupplierProfilePage() {
  ...
  useEffect(() => {
    const fetchSupplierProfile = async () => {
      if (session?.user?.profileId) {
        try {
          const res = await fetch(`/api/supplier/${session.user.profileId}`)
          if (res.ok) {
  ...
  return (
    <ProfileWrapper requiredRole="supplier">
            ...
            <Card.Body>
              <Form>
                ...
                <Form.Group className="mb-3">
                  ...
                  <Form.Text className="text-warning">Leave empty to keep your current product catalog.</Form.Text>
                </Form.Group>
